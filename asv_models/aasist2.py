...
class SSLModel(nn.Module):
    def __init__(self, device, wav2net2_path):
        super(SSLModel, self).__init__()
        """
        Change the pre-trained XLSR model path.
        """
        # xlsr2_300m.pt download link: https://dl.fbaipublicfiles.com/fairseq/wav2vec/xlsr2_300m.pt
        print(wav2net2_path, type(wav2net2_path))
        model, cfg, task = checkpoint_utils.load_model_ensemble_and_task(
            [wav2net2_path]
        )
        # model, cfg, task=fairseq.fairseq.checkpoint_utils.load_model_ensemble_and_task([cp_path])
        self.model = model[0]
        self.device = device
        self.out_dim = 1024
        return
    ...
    def embedding(self, x):
        x = self.extract_feat(x.squeeze(-1))
        audio_emb = torch.mean(x, dim = 1)
        return audio_emb
...
class Model(nn.Module):
    ...
    def __init__(self, device, wav2net2_path):
        ...
        ####
        self.ssl_model = SSLModel(self.device, str(wav2net2_path))
